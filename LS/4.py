...
def main():
     ...
     # naming the y axis
     plt.ylabel('y - axis')
     plt.plot(X[0],Y[0],marker='s',markersize=1,color='#aa2889',label='class 1')
     plt.plot(X[1],Y[1],marker='s',markersize=1,color='#76dc0a',label='class 2')
     plt.plot(X[2],Y[2],marker='s',markersize=1,color='#2391c8',label='class 3')
     ...
     for i in range(3):
         c=[0,0,0]
         for j in range(125):
             G[k]=calculatingG(W[k],w[k],W0[k],[testing_data_X[i][j],testing_data_Y[i][j]])
             c[G.index(max(G))]+=1
         # print c
         for j in range(3):
             confusion_matrix[i][j]=c[j]
